...
#     def __init__(self, config: T5Config):
#         super().__init__()
#         # self.dense_0 = nn.Linear(config.d_model, config.d_model)
#         # self.norm_0 = T5LayerNorm(config.d_model)
...
#         # self.dense_4 = nn.Linear(config.d_model // 16, config.d_model // 32)
#         # self.norm_4 = T5LayerNorm(config.d_model // 32)
#         # self.relu_4 = NewGELUActivation()
#         # self.dropout_4 = nn.Dropout(p=config.classifier_dropout)

#         self.out_proj = nn.Linear(config.d_model // 64, config.num_labels)

#     def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
#         hidden_states = self.relu_0(
#             self.dropout_0(self.norm_0(self.dense_0(hidden_states)))
#         )
#         hidden_states = self.relu_1(
#             self.dropout_1(self.norm_1(self.dense_1(hidden_states)))
#         )
#         hidden_states = self.relu_2(
#             self.dropout_2(self.norm_2(self.dense_2(hidden_states)))
#         )
#         # hidden_states = self.relu_3(
#         #     self.dropout_3(self.norm_3(self.dense_3(hidden_states)))
#         # )
#         # hidden_states = self.relu_4(
#         #     self.dropout_4(self.norm_4(self.dense_4(hidden_states)))
#         # )
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            module.dense_0.weight.data.normal_(
                mean=0.0, std=factor * ((self.config.d_model) ** -0.5)
            )
            if hasattr(module.dense_0, "bias") and module.dense_0.bias is not None:
                module.dense_0.bias.data.zero_()

            module.dense_1.weight.data.normal_(
                mean=0.0, std=factor * ((self.config.d_model // 4) ** -0.5)
            )
            if hasattr(module.dense_1, "bias") and module.dense_1.bias is not None:
                module.dense_1.bias.data.zero_()

            module.dense_2.weight.data.normal_(
                mean=0.0, std=factor * ((self.config.d_model // 16) ** -0.5)
            )
            if hasattr(module.dense_2, "bias") and module.dense_2.bias is not None:
                module.dense_2.bias.data.zero_()

            # module.dense_3.weight.data.normal_(
            #     mean=0.0, std=factor * ((self.config.d_model // 16) ** -0.5)
            # )
            # if hasattr(module.dense_3, "bias") and module.dense_3.bias is not None:
            #     module.dense_3.bias.data.zero_()

            # module.dense_4.weight.data.normal_(
            #     mean=0.0, std=factor * ((self.config.d_model // 16) ** -0.5)
            # )
            # if hasattr(module.dense_4, "bias") and module.dense_4.bias is not None:
            #     module.dense_4.bias.data.zero_()

            # module.dense_5.weight.data.normal_(
            #     mean=0.0, std=factor * ((self.config.d_model // 32) ** -0.5)
            # )
            # if hasattr(module.dense_5, "bias") and module.dense_5.bias is not None:
            #     module.dense_5.bias.data.zero_()

            module.out_proj.weight.data.normal_(
                mean=0.0, std=factor * ((self.config.d_model // 64) ** -0.5)
            )
...
#     )
#     xcs[torch.eye(features.size(0)).bool()] = float("-inf")

#     # create labels mask
#     labels = labels.unsqueeze(0)
#     target = labels.eq(labels.T).float()
#     target[torch.eye(features.size(0)).bool()] = 0

#     return torch.nn.functional.cross_entropy(
#         (xcs / temperature).sigmoid(), target, reduction="mean"
#     )


def nt_xent_loss(features, labels):
    assert len(features.size()) == 2
    ...
    xcs = (xcs + 1) / 2
    xcs[torch.eye(features.size(0)).bool()] = 0

    labels = labels.unsqueeze(0)
    target = labels.eq(labels.T).float()
    target[torch.eye(features.size(0)).bool()] = 0

    return torch.nn.functional.cross_entropy(xcs, target, reduction="mean")
...
def contrastive_learning_loss(
    ...
    """
    Computes contrastive loss using logits and labels.

    Args:
        logits (torch.Tensor): Model output embeddings of shape (batch_size, embedding_dim).
        labels (torch.Tensor): Corresponding class labels of shape (batch_size,).
        temperature (float): Temperature parameter to scale similarity (default: 0.5).

    Returns:
        torch.Tensor: Scalar loss value for the batch.
    """
    # Normalize logits to unit vectors
    logits_ = torch.nn.functional.normalize(logits, p=2, dim=1)

    # Compute pairwise cosine similarity
    pairwise_similarities = torch.mm(logits_, logits_.T) / temperature

    # Exponential similarities for contrastive learning
    exp_similarities = torch.exp(pairwise_similarities)

    # Create positive and negative masks
    labels = labels.unsqueeze(1)  # Shape: (batch_size, 1)
    positive_mask = labels.eq(
        labels.T
    ).float()  # Positive pairs: 1 if labels match, else 0

    # Mask to exclude self-similarity (diagonal elements)
    mask = torch.eye(logits_.size(0), device=logits_.device)
    positive_mask = positive_mask * (1 - mask)

    # Compute contrastive loss
    # For each example: log(sum(exp_similarities for positives) / sum(exp_similarities for all))
    numerator = exp_similarities * positive_mask
    denominator = exp_similarities.sum(
        dim=1, keepdim=True
    ) - exp_similarities.diag().view(-1, 1)

    loss = -torch.log((numerator.sum(dim=1) + 1e-8) / (denominator + 1e-8).squeeze())
    return loss.mean()
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                # contrastive_loss = contrastive_learning_loss(logits, labels)

                # loss = contrastive_loss * 0.1 + cls_loss * 0.9
                loss = cls_loss
